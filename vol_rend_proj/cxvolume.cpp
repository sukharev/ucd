...
#include "cutil.h"
#include <cuda.h>
extern "C" void runTest1(int N, int M,float* h_A, float* h_B, float* h_C );
...
#define NEW_CODE
#define TOTAL_CORREL_CALC	100
#define TS_USED_IN_CORREL_CALC	6
...
#ifdef NEW_CODE
bool cxVolume::vartominmax(int var, float* pdata, float* arr_fMin, 
...
cxVolume::cxVolume(int volID)
{
	...
	*/
	...
	m_nNumTimePearson = TS_USED_IN_CORREL_CALC;//3;//32;
	//m_nStartTimePearson = 0;
...
bool cxVolume::calculate_corr(const char* filename, string var_names[],  //requested variable names
						   int num_vars)			//number of variables requested (no more than 2)
{
	...
	xlocation = (int)(m_corrx * (float)(m_vSize[0]-1));
	ylocation = (int)(m_corry * (float)(m_vSize[1]-1));
	zlocation = (int)(m_corrz * (float)(m_vSize[2]-1));
	...
	if(num_vars == 1){ //self-correlation
		...
		float* points = new float[m_nNumTimePearson*m_nVolumeSize];
		if(!points)
			bUseInCore = false;
		if(bUseInCore){
			...
			bUseInCore = m_netcdf->readtac(filename, var_names[0].c_str(), refpt, 
								refid,	refid, 
								TOTAL_CORREL_CALC, TS_USED_IN_CORREL_CALC);
			...
			vartominmax(m_nVolumeType, points, arr_fMin, arr_fMax, 0, TS_USED_IN_CORREL_CALC,m_nVolumeSize);
			vartominmax(m_nVolumeType, refpt, arr_fMin, arr_fMax, 0, TS_USED_IN_CORREL_CALC,1);

			if(bUseInCore){
				//CUDA Host code
				// run the device part of the program
				runTest1((int)TS_USED_IN_CORREL_CALC, m_nVolumeSize, points, refpt, m_pVolume);
				delete [] refpt;
				delete [] points;
			}
      ...
      bool bCorrelation)
{
	...
	if(!bCorrelation){
		...
		int n = 0;
		while(n < num_vars){
			bTACexists = m_netcdf->checktacfile(m_netcdf->m_filename,m_vars[n].c_str());
			if(!bTACexists)
				break;
			n++;
		}
		if(!bTACexists){
			ReadNetCDF3(m_netcdf, m_time_in, TOTAL_CORREL_CALC /*m_nNumTimePearson*/, m_vars, num_vars, true);
		}
		else{
			int dim[3];
			ClearVolTex();
...
void cxVolume::SetCorrCenterPoint(int axis, float x, float y)
{
	...
	if(m_netcdf){
		Read(m_vars[0].c_str(),m_vars[1].c_str(),true);
	}
	ReDraw(true);
}
/*
