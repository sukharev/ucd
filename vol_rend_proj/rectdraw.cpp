...
cxColor RectDraw::getcolor(int pos)
{
...
float RectDraw::getheight(int pos)
{
	return m_line[pos];
}

cxColor RectDraw::getbackcolor(int pos)
{
	vector<int>::iterator pf;
	int i =0;
	for(pf = m_colorpos.begin();pf!=m_colorpos.end(); pf++, i++)
...
unsigned int * RectDraw::getStatistic()
{
// 	if(m_pParent != NULL)
// 	{
...
float RectDraw::FindPos(int pos)
{
	vector<MyDragButton *>::iterator pb;
	for(pb=m_pParent->m_colors.begin(); pb!=m_pParent->m_colors.end(); pb ++)
	{
...
cxColor RectDraw::FindColor(int pos)
{
	vector<MyDragButton *>::iterator pb;
	for(pb=m_pParent->m_colors.begin(); pb!=m_pParent->m_colors.end(); pb ++)
	{
...
void RectDraw::initline(bool is_erase)
{
    int tx = Fl::event_x();
    int ty = Fl::event_y();
